...
\newTxt{HENS and IFS are distinct modeling systems with different climatological biases, and Figure 1 compares the summertime maximum of each, which is not necessarily from the same event at the same time. A larger ensemble increases the chance of sampling a rare tail event, but it does not override a systematic offset in the underlying model. If IFS has a warmer ensemble mean forecas  for any one particular forecast, or a sharper representation of a particular extreme, it can produce a warmer grid-cell maximum even with far fewer members. In particular, ERA5 (the training dataset for HENS) and IFS have different spectra. ERA5 has a somewhat smoother energy spectrum than IFS (see Fig. S17 of \citet{Zhong2025} and Appendix A of \citet{Mahesh2024hugeensemblesidesign} for a demo) and damps the sharpest small-scale extremes on a per-member basis. HENS's advantage therefore derives from ensemble size sampling the tail.  The difference between the maxima is the expected behavior of two different systems, not evidence of an error.}
