...
\newTxt{To clarify these points in the manuscript, we have added a paragraph to lines 182-203.}
...
\newTxt{Crucially, our central claim does not rest on the raw maximum comparison, because that comparison confounds ensemble size with model bias. We address this with two complementary controls. First, and most directly relevant to the reviewer's concern about differing physics: in Figures 2 and S8 we subsample HENS to a 58-member ensemble and apply the same GEV extrapolation used for IFS (we call this HENS-58). Because HENS-58 and HENS share the same modeling system, this comparison removes the confounding factors of different models entirely. HENS-58 and HENS are an apples-to-apples comparison, and the only difference between them is ensemble size. In the update manuscript, we find that full HENS still exceeds the HENS-58 GEV extrapolation (Figure 1), which demonstrates that the huge ensemble reaches tail states that GEV fitting to a small ensemble of the same model cannot recover. Whatever differences exist between HENS and IFS physics, they cannot explain this gap.}
   ...
   % \newTxt{Finally, the reviewer correctly notes that many of the cells where IFS exceeds HENS fall in the Southern Hemisphere, where June–August is austral winter. Because our study targets boreal-summer heat extremes, the "summertime maximum" in these cells is a winter value and not one of the dangerous heat events the paper is concerned with. Differences between IFS and HENS in this off-season regime are therefore both expected, neither model is being pushed toward its warm tailand largely inconsequential for our conclusions. We will state this explicitly in the revised text so that the SH exceedances are read as an off-season model comparison rather than a comparison of extreme heat.}
    ...
    \newTxt{Thank you for this feedback. In response to your comments we have now added an extensive vetting of using the GEV distribution for both the IFS and HENS-58 subset ensemble (main text lines 271-284 and supplement section 2.3). Ultimately, the new analysis gives us high confidence that GEV theory has been appropriately applied and provides a firm foundation for all future statements regarding the GEV analysis.}
