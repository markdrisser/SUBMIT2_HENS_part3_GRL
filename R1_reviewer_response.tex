...
\vskip1ex
\noindent \textcolor{cadmiumgreen}{In addition to this response document, we also provide a track changes version of the manuscript and a ``final'' version without tracked changes. All line numbers in the rest of this document correspond to the final version \textbf{with tracked changes}. }
...
\newTxt{\sl We also restrict our analysis to a single ML emulator, carefully designed and validated to be fit-for-purpose for this science question. Exploring other models is nontrivial: they are generally less validated, lack a readily available ensemble analogue, and in any case share SFNO's characteristics (same training data, same spatial and temporal resolution). Future huge ensembles should use ML models trained end-to-end as ensemble forecasts, evaluated with the extreme diagnostics pipeline of \cite{Mahesh2024hugeensemblesidesign} before such experiments are conducted.}
\vspace{1ex}
...
\newTxt{To clarify these points in the manuscript, we have added a paragraph to lines 181-200.}
...
\newTxt{HENS and IFS are distinct modeling systems with different climatological biases, and the original Figure 1 compared the summertime maximum of each, which is not necessarily from the same event at the same time. A larger ensemble increases the chance of sampling a rare tail event, but it does not override a systematic offset in the underlying model. If IFS has a warmer ensemble mean forecast for any one particular forecast, or a sharper representation of a particular extreme, it can produce a warmer grid-cell maximum even with far fewer members. In particular, ERA5 (the training dataset for HENS) and IFS have different spectra. ERA5 has a somewhat smoother energy spectrum than IFS (see Fig. S17 of \citet{Zhong2025} and Appendix A of \citet{Mahesh2024hugeensemblesidesign} for a demonstration) and damps the sharpest small-scale extremes on a per-member basis. HENS's advantage therefore derives from ensemble size sampling the tail.}

\newTxt{The difference between the maxima is the expected behavior of two different systems, not evidence of an error.  Further evidence of this is that the GEV scale and shape parameters from HENS-58 and IFS are statistically indistinguishable. Only the location parameters differ, so the differences in ensemble maxima reflect systematic shifts between the two systems rather than differing tail behavior.}
...
\begin{enumerate}
    ...
    \newTxt{Thank you for raising this point. We have now included a reference to \cite{Gyuleva2025} acknowleding their findings on line 80.}
    ...
    \newTxt{Thank you for this feedback. We have changed this sentence to begin with Since the 2023 record temperatures have already been exceeded by subsequent summers, ...'' (line 433).}
   ...
   \newTxt{We thank the reviewer for their question and appreciate the chance to clarify our intention.  We apologize for the ambiguity around phrases such as ``unexplored weather states" and have removed them.  We originally meant to say that HENS can sample beyond the IFS ensemble maximum and the IFS-GEV estimate of the maximum.  The rephrased paragraph is in lines 436-444.}
