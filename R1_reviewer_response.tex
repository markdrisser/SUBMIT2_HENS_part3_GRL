...
\newTxt{We appreciate the reviewer's raising the issue of HENS's validity at the tail of the distributions, particularly concerning the ``exceptionally likely'' temperatures occuring primarily at high latitudes (Russia, Alaska, and Canada) but with contributions from, e.g., northern Africa.  First, we should note that the first of the two HENS papers cited in this manuscript (Mahesh et al., 20225a, \href{https://doi.org/10.5194/gmd-18-5575-2025}{10.5194/gmd-18-5575-2025}) goes well beyond the assertion that we only checked ``bulk level validation of SFNO -- i.e, good performance on standard average forecast-skill metrics'' We refer the reviewer in particular to $\S$3.3 in the first HENS paper concerning ``Extreme diagnostics,'' where we verify that SFNO exhibits excellent performance on the same diagnostics used by ECMWF and NCEP \textbf{specifically} to evaluate their NWP forecasts of extremes, namely :
\begin{itemize}
