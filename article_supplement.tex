...
\section{Integrated Forecast System data}
...
\begin{equation} \label{eq_priors}
    \begin{array}{rcl}
        ...
        p(\xi_i) & \propto & \exp\{-0.577215(1+\xi_i)\}, \hskip3ex \xi_i \in (-0.5, 0).
    \end{array}
\end{equation}
\citep{Zhang2024}. \newTxt{Note that the prior for the location parameter is an improper prior (i.e., its probability density function does not integrate to 1) and imposes no assumptions on the values it may take on. This nonetheless yields valid posterior inference.} For simplicity, we apply this statistical model separately to each \st{IFS} \newTxt{$0.25^\circ \times 0.25^\circ$} grid box.
...
\begin{figure}[!h]
    \centering
\includegraphics[width = \textwidth]{Figure_HENS58GEV_calib.png}
    \caption{Caption}
    \label{fig:hens58calib}
\end{figure}


\begin{figure}[!h]
    \centering
\includegraphics[width = \textwidth]{Figure_t2m_probContain_1000day_HENS58GEV_vs_HENS.png}
    \caption{As in Figure 3 in the main text, comparing HENS-58-GEV and HENS.}
    \label{fig:hens58gev_vs_hens}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=1.05\linewidth]{Figure_Slide_36_by_Ankur_Misses_Ground_Truth.png}
    \caption{
    % (Figure 36 of \texttt{HENS Part 3\_ initial review}.) 
    Percentage of 10-day forecasts initialized during summer 2023 when the ensemble-maximum 2m air temperature is less than the verification value.   We define this condition as a ``miss.'' IFS is shown on the left panel, and HENS is shown on the right.  Over global land, IFS and HENS have miss rates of 5.8\% and 0.6\%, respectively. For a fair comparison, the verification value for HENS is derived from ERA5, and the verification value for IFS is obtained from operational analysis.
    \label{fig:Misses_Ground_Truth}}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=1.05\linewidth]{Figure_Slide_37_by_Ankur_IFS_vs_ERA5_bias_in_max.png}
    % \caption{(Figure 37 of \texttt{HENS Part 3\_ initial review}.) For summer 2023, the time-mean differences between the instantaneous ensemble-maximum 2m air temperature at 10-day lead times and the corresponding temperatures from ERA5 at the verification times of these forecasts/hindcasts.
    \caption{At the time of the ERA5 summer 2023 maximum temperature, the difference between the verification value and the 10-day HENS and IFS ensemble maxima is shown. For a fair comparison, the verification value for HENS is derived from ERA5, and the verification value for IFS is obtained from operational analysis.
    % instantaneous ERA5 maximum 2m temperature at 10-day lead times and the corresponding temperatures }
    \label{fig:IFS_bias_vs_ERA5}}
\end{figure}
